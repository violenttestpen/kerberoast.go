...
import (
	...
	"github.com/violenttestpen/kerberoast.go/hmac"
...
func Decrypt(key []byte, messagetype int, edata []byte) (data []byte, nonce []byte, err error) {
	...
	// Calculate checksum
	checksum := hmac.CalculateHMACMD5(K1, *ddata)

	if bytes.Equal(checksum, edata[:16]) {
		data, nonce = (*ddata)[8:], (*ddata)[:8]
	} else {
		err = ErrChecksum
	}
	return
